...
Options:
    --pca-dir DIR            PCA artifacts directory (default: pca/pca_results)
    --models-dir DIR         Trained models directory (default: xgmodels/20modes)
    --results-dir DIR        Output directory (default: predict_results)
    --q-min Q                Exclude |q| < Q (beamstop mask)
    --q-max Q                Exclude |q| > Q from fit
    --no-rescale             Skip affine rescaling (input already on simulation scale)
    --rescale-background BG  Subtract constant BG before fit (same units as intensity)
    --beta BETA              RPA structure factor: model-side S(q) = 1/(1+beta*P(q))
    --ridge-lambda LAMBDA    Ridge regularization strength used in affine rescaling/PCA projection
    --smooth-sigma SIGMA     Mask-normalized smoothing on aligned intensity grid
"""
...
def project_with_rescaling(
    ...
    + lam*||alpha||^2. Returns (alpha, scale, background, n_function_evaluations, fit_info).
    """
    k_full = U.shape[1]
    ...
    scale0 = model_med / p_med if p_med != 0.0 else 1.0
    if not np.isfinite(scale0) or scale0 == 0.0:
        scale0 = 1.0
    bg0 = 0.0 if fixed_background is not None else model_med - scale0 * p_med
    ...
    if fixed_background is None:
        def residual(x: np.ndarray) -> np.ndarray:
            scale, background = float(x[0]), float(x[1])
            alpha = x[2:]
        ...
        x0 = np.concatenate(([scale0, bg0], np.zeros(k_full)))
        result = least_squares(
            residual,
            x0=x0,
            max_nfev=max_iter,
        ...
        scale, background = float(result.x[0]), float(result.x[1])
        alpha = result.x[2:].reshape(1, -1)
        alpha_slice = slice(2, 2 + k_full)
    else:
        ...
        x0 = np.concatenate(([scale0], np.zeros(k_full)))
        result = least_squares(
            residual,
            x0=x0,
            max_nfev=max_iter,
        ...
        alpha = result.x[1:].reshape(1, -1)
        alpha_slice = slice(1, 1 + k_full)
...
def main() -> None:
    ...
    parser.add_argument("--pca-dir", type=str, default="pca/pca3m", dest="pca_dir")
    parser.add_argument("--results-dir", type=str, default="predict_results", dest="results_dir")
    parser.add_argument("--models-dir", type=str, default="xgmodels/10modes3m")
    parser.add_argument("--no-rescale", action="store_true", help="Skip affine rescaling")
    parser.add_argument("--q-min", type=float, default=None, help="Beamstop radius (exclude |q| < value)")
    ...
    parser.add_argument("--rescale-background", type=float, default=None,
                        help="Constant background to subtract before fit")
    parser.add_argument("--beta", type=float, default=None,
                        help="RPA structure factor beta (model-side S = 1/(1+beta*P))")
    ...
    if args.no_rescale:
        ...
        alpha_full, rescale_scale, rescale_bg, fit_evals, fit_info = project_with_rescaling(
            ...
            fixed_background=fixed_bg, beta=beta,
        )
